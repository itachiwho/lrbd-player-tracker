...
const allowedDomains = [
  ...
  if (!isFromAllowedOrigin && !hasValidToken) {
    console.log(`[Security] Blocked metrics.js - Referer/Origin: "${referer}", Auth: ${authHeader ? 'present' : 'missing'}`);
  ...
  // Log success
  if (hasValidToken) {
    console.log('[Security] metrics.js allowed via Bearer token');
  } else {
    console.log(`[Security] metrics.js allowed via origin: ${referer}`);
  }
